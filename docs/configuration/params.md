...
| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `github_allowed_orgs` | array | none | GitHub organizations whose members may log in, e.g. `[ my-org ]`. Required unless `github_allowed_teams` is set. |
| `github_allowed_teams` | array | none | GitHub `org:team` slugs whose members may log in, e.g. `[ my-org:platform-team ]`. Required unless `github_allowed_orgs` is set. |
